"""
S3에서 Diffusers 모델을 직접 로드하는 유틸리티
...
def load_model_directory_from_s3(
    model_dir_name: str = "sdxl_base",
    bucket_name: Optional[str] = None,
    s3_prefix: Optional[str] = None,
    local_cache_dir: Optional[Path] = None,
) -> str:
    """
    S3에서 Diffusers 모델 디렉토리 전체를 다운로드하여 로컬 경로 반환
    
    Args:
        model_dir_name: 모델 디렉토리 이름 (예: "sdxl_base")
        bucket_name: S3 버킷 이름
        s3_prefix: S3 프리픽스 (예: "models/diffusers/")
    ...
    s3_prefix = s3_prefix or os.getenv("S3_MODEL_PREFIX", "models/diffusers/")
    ...
    if local_cache_dir is None:
        local_cache_dir = Path(tempfile.gettempdir()) / "s3_models" / "diffusers"
    else:
    ...
    # 이미 다운로드되어 있으면 스킵
    if local_model_dir.exists() and (local_model_dir / "model_index.json").exists():
